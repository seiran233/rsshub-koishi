...
export const Config: Schema<Config> = Schema.object({
  RssHubServerUrl: Schema.string()
    .default("http://rsshub.app")
    .description("RssHub服务地址(若是在中国大陆使用，推荐换源或是自建)"),
...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.http.config.baseURL = rsshubServerUrl;
  ctx.puppeteer.config.timeout = 120000
  const mainDoc = `
====rsshub订阅推送插件帮助====
所属指令
...
更多帮助在各个指令末尾加上 -h查看
  ...
  ctx
    .command("rss/rsschannel.subscribe [guildId:text]", "订阅频道")
    .alias("rss.subscribe")
    .alias("rss.订阅")
    .alias("rss.dy")
  ...
  ctx
    .command("rss/rssitem.get <id:number>", "查看项目")
    ...
    .action(async ({ session }, id) => {
      try {
        ...
        const img = await spliceRssItems(items)
        const url = img.match(/src\="(?<url>.*)"/)
        return <message forward={true}>
          <author
            id={session.author.id}
            name={session.author.name}
            avatar={session.author.avatar}
          />
          <file poster={url.groups.url} src={url.groups.url}></file>
        </message>
      } catch (error) {
        logger.error(error);
        if (error instanceof Error) {
  ...
  ctx
    .command(
      "rss/rssitem.list <cid:number> [page:number]",
      "展示某频道下的项目"
    )
    ...
    .action(async ({ session, options }, cid, page) => {
      ...
      const res: RssItem[] = await ctx.database
        .select("RssItem")
        .where({ cid: cid })
      ...
      const itemText: string = res
        .map((item) => `<tr><td>${item.id}</td><td>${item.title}</td></tr>`)
        .join("");
  ...
  ctx
    .command("rss/rsschannel.list", "展示订阅的频道列表")
    ...
    .action(() => {
      ...
      const text = `<p>频道列表</p><table border=0><tr><td>序号</td><td>标题</td></tr>${list.join(
        ""
      )}</table>`;
      return render(text, 250);
    });
  ...
  ctx
    .command("rss/rsschannel.remove <id:number>", "删除频道及其所属的项目")
    ...
    .action(async ({ session }, id) => {
      ...
      session.send(
        `是否永久删除“${channel.title}”以及其下所有的项目?此操作不可逆哦!\n确认请回复“确认”,输入其他或超时则取消`
      );
      try {
  ...
  ctx
    .command("rss/rsschannel.deliver <id:number>", "显示频道的推送目标")
    .alias("rss/rsschannel.推送目标")
    .action(async ({ session }, id) => {
      if (!id) return "指令错误,请输入频道ID";
      try {
        const channel = (await ctx.database.get("RssChannel", id)).pop();
        const res = channel.deliver.map(
  ...
  ctx
    .command(
      "rss/rsschannel.deliverto <id:number> <guildId:text>",
      "修改推送目标群"
    )
  ...
  class UI {
    ...
    getDeliver(rawGuildId?: string, selfDeliver = false): Deliver {
      ...
      if (!rawGuildId && selfDeliver)
        throw new Error("-d 时必须要指定目标群聊");
      return deliver;
    }
  ...
  async function rssDiv(rssItem: RssItem) {
    const channel = `<h2>#${rssItem.id}:${(await getChannelFromItem(rssItem)).title
      }</h2>`;
...
<body>
  ...
  async function createRssItemList(channel: RssChannel): Promise<RssItem[]> {
    ...
    for (const item of items) {
      const archivedItems = await ctx.database.get("RssItem", {
    ...
    return Promise.all(
      newItems.map((item) => CreateRssItem(ctx, item, channel))
    );
  }

  async function subscribeRssChannel(
    session: Session,
    type: RssChannelType,
    deliver: Deliver
  ): Promise<RssChannel> {
    ...
    const channel = await channelFactory.createChannel("", args, deliver);
    channelList.push(channel);
    return channel;
  }

  async function checkForUpdates(): Promise<
    { channel: RssChannel; items: RssItem[] }[]
  > {
    logger.info("开始更新");
    let res = await Promise.all(
      channelList.map(async (channel: RssChannel) => {
        const items = await createRssItemList(channel);
